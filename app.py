...
def update_output(x_value,par,mode,state,padd):
    ...
    fig3.add_trace(
        go.Scatter(x=production.index, y=production[padd],name='Net Production', marker_color='green'),
        secondary_y=True
    )
    fig3.update_xaxes(rangeslider_visible = True)
    fig3.update_yaxes(title_text="Gas Price", secondary_y=False)
    fig3.update_yaxes(title_text='Net Production', secondary_y=True)
    fig3.update_layout(
        title={
            'text': padd + ' Yearly Average Gas Price VS Net Production',
            'font_size': 30,
            'x':0.5,
            'xanchor': 'center',
            'yanchor': 'top'
